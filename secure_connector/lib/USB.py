...
class Device:
    ...
    def __init__(self, device_name, device_type,
                 device_return_string, device_baudrate, label=False):
        ...
        # If thread is not a daemon application, you won't be able to exit it
        search_thread.daemon = True
        search_thread.start()

    # Display status on the command line or inside a tkinter application
    ...
    # Detect Arduino Nano Clone identity (or any serial port)
    ...
    def search(self):
        if not self.isConnected:
            ...
            for device in devices:
                ...
                # If we found the correct arduino
                # we return the serial connection
                if self.isConnected:
